...
<div id="main">
    ...
    <div id="page">
        <?php
        if(!empty($message)){ //we no longer need to pull message from a session cookie, look above
            echo "<div class=\"message\">".htmlentities($message)."</div>";
        }
        echo form_errors($errors);
        ?>

        <h2>Edit Subject/Menu Name: <?php echo htmlentities($current_subject["menu_name"]);?></h2>
        <form action="edit_subject.php?subject=<?php echo $current_subject["id"];?>" method="post">
            ...
            <p>Position:
                <select name="position">
                    ...
                    for($count = 1; $count <= $subject_count; $count++){ //+1 because we want to allow to add a subject
